  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-base font-semibold text-zinc-900">Logi SMS ({total})</h2>
        <div className="flex gap-1 bg-brand-50 p-0.5 rounded-lg">
          {(["all", "sent", "failed"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              className={`px-3 py-1.5 text-xs font-medium rounded-md transition ${filter === f ? "bg-white text-zinc-900 shadow-sm" : "text-zinc-500 hover:text-zinc-700"}`}>
      ...
      {loading ? (
        <p className="text-center text-zinc-400 py-8">Ładowanie...</p>
      ) : logs.length === 0 ? (
        <div className="border border-dashed border-zinc-200 rounded-xl p-8 text-center">
          <p className="text-sm text-zinc-400">Brak SMS-ów</p>
        </div>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white text-left">
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-500 uppercase">Firma</th>
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-500 uppercase">Numer</th>
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-500 uppercase">Treść</th>
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-500 uppercase">Status</th>
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-500 uppercase">Wysłano</th>
                  <th className="px-4 py-3 text-xs font-semibold text-zinc-500 uppercase">Błąd</th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log) => (
