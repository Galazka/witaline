...
export default function RecentCallsPreview({ callLogs = [], onClick }: Props) {
  const safeLogs = callLogs || [];
  const totalSeconds = safeLogs.reduce((acc, l) => acc + l.duration_seconds, 0);
  const totalCost = safeLogs.reduce((acc, l) => acc + l.cost_pln, 0);
  const ordersCount = safeLogs.filter((l) => l.classification === "order").length;
  const helpfulCount = safeLogs.filter((l) => l.was_helpful === true).length;
  const avgDuration = safeLogs.length > 0 ? Math.round(totalSeconds / safeLogs.length) : 0;

  const recentCalls = safeLogs.slice(0, 5);
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        <div className="bg-white/55 dark:bg-brand-800/55 backdrop-blur-xl rounded-xl border border-white/20 dark:border-white/5 p-3">
          <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Wszystkie</p>
          <p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">{safeLogs.length}</p>
        </div>
        <div className="bg-white/55 dark:bg-brand-800/55 backdrop-blur-xl rounded-xl border border-white/20 dark:border-white/5 p-3">
          <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Zamówienia</p>
          <p className="text-lg font-bold text-[#0d9488]">{ordersCount}</p>
        </div>
        <div className="bg-white/55 dark:bg-brand-800/55 backdrop-blur-xl rounded-xl border border-white/20 dark:border-white/5 p-3">
          <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Śr. czas</p>
          <p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">{avgDuration}s</p>
        </div>
        <div className="bg-white/55 dark:bg-brand-800/55 backdrop-blur-xl rounded-xl border border-white/20 dark:border-white/5 p-3">
          <p className="text-[10px] text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">Koszt</p>
          <p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">{totalCost.toFixed(2)} zł</p>
        </div>
      </div>
        ...
        <div className="bg-white/55 backdrop-blur-xl rounded-lg p-3 mb-3">
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="text-green-600 font-medium">{helpfulCount} pozytywnych</span>
              ...
              <div className="h-full bg-[#0d9488] rounded-full" style={{ width: `${(ordersCount / Math.max(safeLogs.length, 1)) * 100}%` }} />
            </div>
            <span className="text-zinc-500">{Math.round((ordersCount / Math.max(safeLogs.length, 1)) * 100)}%</span>
          </div>
        </div>
      )}
