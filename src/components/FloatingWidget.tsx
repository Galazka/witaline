...
export default function FloatingWidget() {
  ...
  const pulseRingClass = !open ? "before:absolute before:inset-0 before:rounded-full before:animate-ping before:bg-[#0d9488]/30" : "";
    ...
    <>
      ...
      >
        ...
        {!consented && (
          <div className="p-6 text-center space-y-5 min-h-[320px] flex flex-col items-center justify-center">
            ...
            <p className="text-[10px] text-zinc-400 select-text">
              Pelna tresc: <a href="/polityka-prywatnosci" target="_blank" className="text-[#0d9488] underline font-medium">Polityka prywatnosci</a>
            </p>
            <button onClick={() => setConsented(true)} className="bg-[#0d9488] text-white w-full py-3 rounded-2xl font-semibold text-sm hover:bg-[#0f766e] transition-colors shadow-sm cursor-pointer">
        ...
        {consented && (
          <>
            {/* Header */}
            <div className="bg-gradient-to-r from-[#0f766e] to-[#0d3d3a] px-5 py-4">
              ...
              <>
                <div ref={chatRef} className="overflow-y-auto px-4 py-4 space-y-3 bg-white/50 min-h-[280px] max-h-[360px]">
                  {messages.map((msg, i) => (
                    <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                      ...
                      <div className={`max-w-[82%] px-4 py-2.5 text-sm leading-relaxed ${msg.role === "user" ? "bg-[#0d9488] text-white rounded-2xl rounded-br-md" : "bg-white text-zinc-800 rounded-2xl rounded-bl-md border border-zinc-100 shadow-sm"}`}>
                        {msg.text}
                      </div>
                    </div>
                  ))}
                ...
                {messages.length <= 2 && (
                  <div className="px-4 pb-2 pt-1 flex flex-wrap gap-1.5">
                    {SUGGESTIONS.map((s) => (
                      <button key={s} onClick={() => send(s)} className="text-xs px-3.5 py-1.5 bg-white text-[#0d9488] rounded-full hover:bg-[#f0fdfa] border border-[#0d9488]/20 font-medium transition-colors shadow-sm cursor-pointer">
                        {s}
                      </button>
                    ))}
                  </div>
                ...
                <div className="border-t border-zinc-100 bg-white p-3">
                  <div className="flex gap-2 items-center">
                    <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => { if (e.key === "Enter") send(input); }} placeholder="Napisz wiadomosc..."
                      className="flex-1 px-4 py-2.5 bg-white border border-zinc-200 rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-[#0d9488]/20 focus:border-[#0d9488] transition-all" />
                    <button onClick={() => send(input)} disabled={!input.trim() || typing}
                      className="w-10 h-10 bg-[#0d9488] text-white rounded-full flex items-center justify-center hover:bg-[#0f766e] disabled:opacity-30 shrink-0 transition-colors shadow-sm cursor-pointer">
                  ...
                  <p className="text-center text-[9px] text-zinc-300 mt-2">
                    Powered by <span className="text-[#0d9488] font-semibold">WitaLine</span>
                  </p>
                </div>
              </>
            )}
