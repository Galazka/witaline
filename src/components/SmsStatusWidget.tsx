...
export default function SmsStatusWidget({ businessId }: Props) {
  const [data, setData] = useState<{
    ...
    wa_limit: number; wa_used: number; wa_extra_purchased: number;
  } | null>(null);
  ...
  const waRemaining = Math.max(0, (data.wa_limit || 0) + (data.wa_extra_purchased || 0) - (data.wa_used || 0));
  ...
  return (
    <div className="bg-white rounded-xl p-4 space-y-3">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-lg font-bold ${waRemaining < 5 ? "text-red-500" : "text-brand-400"}`}>{waRemaining}</span>
      </div>
