...
const ratingMeta: Record<number, { emoji: string; label: string; color: string }> = {
  3: { emoji: "😊", label: "Zadowolony", color: "bg-brand-100 text-brand-600" },
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex gap-2 flex-wrap">
          <button onClick={() => setFilter("all")} className={`px-3 py-1.5 text-xs rounded-lg transition ${filter === "all" ? "bg-brand-400 text-white" : "bg-brand-50 text-zinc-600 hover:bg-brand-100"}`}>Wszystkie ({items.length})</button>
          {[3, 2, 1].map(r => (
            <button key={r} onClick={() => setFilter(r)} className={`px-3 py-1.5 text-xs rounded-lg transition ${filter === r ? "bg-brand-400 text-white" : "bg-brand-50 text-zinc-600 hover:bg-brand-100"}`}>
              {ratingMeta[r].emoji} {ratingMeta[r].label} ({counts[r] || 0})
            </button>
          ))}
        </div>
        <div className="flex gap-2">
          <button onClick={() => { setView("active"); setChecked(new Set()); }} className={`px-4 py-2 text-sm font-medium rounded-lg transition ${view === "active" ? "bg-brand-100 text-brand-600" : "text-zinc-500 hover:text-zinc-700"}`}>Aktywne</button>
      ...
      {checked.size > 0 && (
        <div className="flex items-center gap-3 bg-brand-50 rounded-xl px-4 py-3">
          ...
          {view === "active" ? (
            ...
            <>
              <button onClick={() => apiAction([...checked], "restore")} className="text-sm px-3 py-1.5 rounded-lg bg-brand-400 text-white hover:bg-brand-500">Przywróć</button>
            ...
            <div key={item.id} className={`bg-white rounded-xl border p-4 space-y-2 ${item.deleted_at ? "opacity-60 border-zinc-200" : "border-zinc-200"} relative`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <input type="checkbox" checked={checked.has(item.id)} onChange={() => { setChecked(prev => { const n = new Set(prev); if (n.has(item.id)) n.delete(item.id); else n.add(item.id); return n; }); }} className="accent-brand-400 rounded" />
              ...
              <div className="flex gap-1 pt-1">
                {item.deleted_at ? (
                  <>
                    <button onClick={() => apiAction([item.id], "restore")} className="text-[10px] px-2 py-1 rounded bg-brand-100 text-brand-600 hover:bg-brand-200">Przywróć</button>
