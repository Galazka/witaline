...
export default function WidgetPage({ searchParams }: { searchParams: Promise<{ business?: string }> }) {
  ...
  return (
    <div className="h-screen flex flex-col bg-white dark:bg-zinc-950 transition-colors">
      {/* Header */}
      <div className="bg-gradient-to-r from-brand-400 to-brand-500 px-4 sm:px-5 py-3 sm:py-4 flex items-center gap-3 shrink-0">
      ...
      <div className="flex border-b border-zinc-100 dark:border-zinc-800 px-4 pt-2 shrink-0">
        <button onClick={() => setTab("chat")} className={`flex-1 flex items-center justify-center gap-2 text-xs font-semibold py-3 transition-all border-b-2 ${tab === "chat" ? "text-brand-600 dark:text-brand-400 border-brand-400" : "text-zinc-400 dark:text-zinc-500 border-transparent hover:text-zinc-600 dark:hover:text-zinc-300"}`}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" /></svg>
          Czat tekstowy
        </button>
        <button onClick={() => setTab("voice")} className={`flex-1 flex items-center justify-center gap-2 text-xs font-semibold py-3 transition-all border-b-2 ${tab === "voice" ? "text-brand-600 dark:text-brand-400 border-brand-400" : "text-zinc-400 dark:text-zinc-500 border-transparent hover:text-zinc-600 dark:hover:text-zinc-300"}`}>
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
          Zadzwoń
          {callActive && <span className="text-[10px] bg-brand-100 dark:bg-brand-900/50 text-brand-600 dark:text-brand-400 px-1.5 py-0.5 rounded-full font-mono">{formatDuration(callDuration)}</span>}
        </button>
      </div>
          ...
          <div ref={chatRef} className="flex-1 overflow-y-auto px-3 sm:px-4 py-4 space-y-4">
              ...
              <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] sm:max-w-[80%] px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${
                  msg.role === "user"
                    ...
                    : "bg-brand-50 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 rounded-2xl rounded-bl-md"
                }`}>
                  {msg.text}
                </div>
              </div>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-brand-50 dark:bg-zinc-800 px-4 py-3 rounded-2xl rounded-bl-md">
          ...
          {conversationId && !transferRequested && (
            <div className="px-3 sm:px-4 pt-2 shrink-0">
              <button
                ...
                className="w-full text-xs text-zinc-400 dark:text-zinc-500 hover:text-brand-500 dark:hover:text-brand-400 transition flex items-center justify-center gap-1.5 py-2.5 sm:py-2 border border-dashed border-zinc-200 dark:border-zinc-700 rounded-lg hover:border-brand-300 dark:hover:border-brand-600"
              >
          ...
          <div className="border-t border-zinc-100 dark:border-zinc-800 p-3 shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-full text-sm text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-brand-400/30 dark:focus:ring-brand-400/50 transition-colors"
              />
              <button
                onClick={() => handleSend(inputValue)}
                disabled={!inputValue.trim() || isTyping}
                className="w-10 h-10 bg-brand-400 text-white rounded-full flex items-center justify-center hover:bg-brand-500 transition disabled:opacity-30 shrink-0"
              >
