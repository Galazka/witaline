...
export async function POST() {
  ...
  const { data: callLogs } = await supabaseAdmin
    .from("call_logs")
    .select("id, business_id, elevenlabs_conversation_id, twilio_call_sid, cost_elevenlabs, cost_twilio, cost_openrouter, total_cost, revenue_pln, cost_pln, internal_cost_pln, duration_seconds, created_at, routed_to_extension")
  ...
  for (const chunk of chunks) {
    const results = await Promise.allSettled(
      chunk.map(async (log) => {
        ...
        const calcTotalCost = Math.round((currentElevenlabs + currentTwilio + currentOpenrouter + consultantTransferCost) * 100000) / 100000;
